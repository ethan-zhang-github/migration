...
public abstract class ConcurrentMigrationWriter<O> implements MigrationWriter<O> {

    private final int size;

    private final Executor executor;

    public ConcurrentMigrationWriter(int size, Executor executor) {
        this.size = size;
    ...
    public int write(MigrationContext<?, O> context, MigrationChunk<O> output) {
        ...
        BatchUtil.partitionAndProcess(output.toList(), size, (List<O> list) -> counter.addAndGet(writeInternal(context, MigrationChunk.ofList(list))), executor);
        return counter.get();
    }
